...
- **既然内存缓存这么高效，是否能让数据都存放在内存中？**
  - 不可以。内存比硬盘容量小得多，操作系统需精打细算内存的使用，所以能让我们使用的内存不多
- 当访问过页面后，再次刷新页面，可发现很多数据都来自**内存缓存**
- ![img](05 sessionStorage和localStorage.assets/webp.webp)
- ![image-20241027230628578](05 sessionStorage和localStorage.assets/image-20241027230628578.png)
- 内存缓存中有一块重要的缓存资源是`preloader`相关指令（例如`<link rel="prefetch">`）下载的资源
  - preloader的相关指令是页面优化的常见手段之一，它可一边解析js/css文件，一边网络请求下一个资源
- 注意：**内存缓存在缓存资源时并不关心返回资源的HTTP缓存头Cache-Control是什么值，同时资源的匹配也并非仅仅是对URL做匹配，还可能会对Content-Type，CORS等其他特征做校验**。具体理解如下：
  - 内存缓存在缓存资源时，有自己独立的缓存机制，而不依赖于HTTP响应头中的`Cache-Control`指令
  - 通常在浏览器的缓存层级中，内存缓存是一种短期的快速缓存，不受HTTP缓存策略的直接控制
  - 即使HTTP响应头中设置了某些缓存策略（如`Cache-Control: no-store`或`Cache-Control: max-age=0`）,内存缓存也可能会暂时存储这些资源。这是因为内存缓存的主要目的是提高性能，减少频繁访问的资源加载时间。因此内存缓存的决策机制与`Cache-Control`指令无直接关系
  - 当浏览器从内存缓存中查找资源时，不仅仅会匹配资源的URL，还会考虑其他特征，例如：
    - **Content-Type**：资源的内容类型必须一致，避免类型不符的资源被错误缓存或使用。
    - **CORS 规则**：跨域资源会根据其CORS策略进行验证，确保资源在跨域访问时符合安全策略，避免因跨域问题导致的安全隐患
...
- ![image-20241027235155966](05 sessionStorage和localStorage.assets/image-20241027235155966.png)
...
- Push Cache 在国内能查到的资料很少，因为` HTTP/2` 在国内不够普及。推荐阅读`Jake Archibald`的 [HTTP/2 push is tougher than I thought](https://links.jianshu.com/go?to=https%3A%2F%2Fjakearchibald.com%2F2017%2Fh2-push-tougher-than-i-thought%2F) 这篇文章，文章中的几个结论：
