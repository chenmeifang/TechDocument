...
2. `Async` 和 `await`相较于 `*` 和 `yield` 更加语义化
...
**但调用 Generator 函数后，该函数并不执行**，返回的也不是函数运行结果，而是一个指向内部状态的指针对象——[遍历器对象（Iterator Object）](https://es6.ruanyifeng.com/#docs/iterator)

下一步，必须**调用遍历器对象的`next`方法**，使得指针移向下一个状态。也就是说，每次调用`next`方法，内部指针就从函数头部或上一次停下来的地方开始执行，直到遇到下一个`yield`表达式（或`return`语句）为止。换言之，Generator 函数是分段执行的，`yield`表达式是暂停执行的标记，而`next`方法可以恢复执行
...
function* objectEntries(obj) {
  // Reflect.ownKeys()是ES6引入的Reflect对象的一个方法，它会返回对象的所有属性键
  // 包括：1.常规的可枚举属性键(通过Object.keys()获取)
  //      2.Symbol属性键(Object.getOwnPropertySymbols()返回的值)
  //      3.不可枚举属性键
...
# [第一节：编写第一个async-await函数](https://www.bilibili.com/video/BV1xW411J7K6?from=search&seid=10512449033732187172)

ES6----ES2015

async和await是ES2017里面引入的新特性

| 原代码                                                       | 扁平化                                                       |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| <img src="03 async和await.assets/image-20241008163803971.png" alt="image-20241008163803971"  /> | ![image-20241008164027741](03 async和await.assets/image-20241008164027741.png) |
|                                                              | 第6行代码又必要加await吗？——有必要。因为response.json是异步的方法 |

- getZhihuColumn函数的函数体读起来更像是同步的代码，但是await关键字接收一个promise
...
# [第二节：将async函数用在promise chain中](https://www.bilibili.com/video/BV1xW411J7K6/?p=2&spm_id_from=pageDriver&vd_source=a7089a0e007e4167b4a61ef53acc6f7e)

**因为所有async函数都会返回一个promise对象，所以我们可以像使用promise一样来使用async函数的返回值**
...
getZhiuColumn函数返回一个promise。也就意味着我们可以利用promise chain把多个async函数串起来。也可以调用promise的catch方法

|                                                              | 进一步简化                                                   |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| ![image-20241008171021355](03 async和await.assets/image-20241008171021355.png) | ![image-20241008171058917](03 async和await.assets/image-20241008171058917.png) |

# [第三节：把任意类型的函数转成async风格](https://www.bilibili.com/video/BV1xW411J7K6/?p=3&spm_id_from=pageDriver&vd_source=a7089a0e007e4167b4a61ef53acc6f7e)
...
<img src="03 async和await.assets/image-20241008171829256.png" alt="image-20241008171829256" style="zoom:67%;" />

是的，无论是在 Node.js 还是在浏览器环境中，在全局作用域（顶级作用域）下使用 `async` 关键字是非法的。这是因为 `async` 函数声明必须放在函数或模块的作用域中，而不能直接放在顶级的全局作用域下。

在 JavaScript 中，所有 `async` 函数都是异步的，它们返回一个 Promise 对象。由于顶级作用域通常不支持异步行为，将 `async` 直接应用在全局作用域会导致语法错误。
...
然而，在模块（如 ES6 模块或 CommonJS 模块）中，由于它们是文件作用域，可以在顶层使用 `async`。同样的，在顶级作用域中可以使用 IIFE（立即执行函数表达式）来绕过限制：
...
这个限制主要是为了防止代码结构混乱，并确保异步操作在合适的上下文中运行。

# [第四节： 处理async函数中的错误](https://www.bilibili.com/video/BV1xW411J7K6/?p=4&spm_id_from=pageDriver&vd_source=a7089a0e007e4167b4a61ef53acc6f7e)

<img src="03 async和await.assets/image-20241008173106814.png" alt="image-20241008173106814" style="zoom: 67%;" />

# [第五节：正确处理多个await操作的并行串行](https://www.bilibili.com/video/BV1xW411J7K6/?p=5&spm_id_from=pageDriver&vd_source=a7089a0e007e4167b4a61ef53acc6f7e)

第10行和第11行这两个请求是串行的：等待第一个请求完成之后，然后发起第二个请求。

<img src="03 async和await.assets/image-20241008173819101.png" alt="image-20241008173819101" style="zoom:50%;" />

如何使用async和await，但同时保证这两个请求是并行的呢？？

<img src="03 async和await.assets/image-20241008173922249.png" alt="image-20241008173922249" style="zoom:50%;" />

# [第六节：使用Promise.all( )让多个await操作并行](https://www.bilibili.com/video/BV1xW411J7K6/?p=6&spm_id_from=pageDriver&vd_source=a7089a0e007e4167b4a61ef53acc6f7e)

<img src="03 async和await.assets/image-20241008174436212.png" alt="image-20241008174436212" style="zoom:50%;" />

# [第七节：结合await和任意兼容.then()的代码](https://www.bilibili.com/video/BV1xW411J7K6/?p=7&spm_id_from=pageDriver&vd_source=a7089a0e007e4167b4a61ef53acc6f7e)

await操作符后面通常会跟一个promise, 如果后面跟的不是promise，会怎么样？
...
# [第八节：在for循环中正确使用await](https://www.bilibili.com/video/BV1xW411J7K6/?p=8&spm_id_from=pageDriver&vd_source=a7089a0e007e4167b4a61ef53acc6f7e)

|                         在循环中串行                         | 在循环中并行                                                 |
| :----------------------------------------------------------: | ------------------------------------------------------------ |
| <img src="03 async和await.assets/image-20241008180004791.png" alt="image-20241008180004791"  /> | ![image-20241008180317764](03 async和await.assets/image-20241008180317764.png) |
| 上面这种写法是串行的！！！！！<br />如何把循环里面的串行改成并行，让代码运行的更快？？？<br />思路：先触发所有的请求，然后拿到一个promise的数组，然后遍历这个数组，等待里面的结果。 |                                                              |
| ![image-20241008180337569](03 async和await.assets/image-20241008180337569.png) | ![image-20241008180346085](03 async和await.assets/image-20241008180346085.png) |

# console.time和console.timeEnd

`console.time()` 和 `console.timeEnd()` 是 JavaScript 中用于测量代码执行时间的内置方法。它们帮助开发者衡量一段代码的执行时间，有助于进行性能优化。

### 使用方法：
1. **`console.time(label)`**：启动一个计时器。参数 `label` 是一个字符串，用来标记计时器的名字。这个计时器的名字可以帮助你区分不同的计时器。
2. **`console.timeEnd(label)`**：停止计时器并输出计时结果。它会输出自 `console.time(label)` 启动以来的时间差，并在控制台打印出来。
...
console.timeEnd('myTimer');  // 停止计时并输出结果
```

在这个例子中，`console.time('myTimer')` 启动了名为 `'myTimer'` 的计时器，随后执行一个循环操作，最后通过 `console.timeEnd('myTimer')` 停止计时并输出执行时间。控制台将会显示类似以下的输出：
...
### 注意事项：
- `label` 是区分多个计时器的标识，必须一致才能正确匹配到相应的 `console.timeEnd()`。如果 `label` 不匹配，则 `console.timeEnd()` 无法停止计时。
- 如果不传 `label` 参数，默认会使用 `'default'` 作为计时器名称。

### 适用场景：
- **性能分析**：用于找出性能瓶颈，衡量函数或代码块的执行时间。
- **调试**：方便开发者在调试时快速查看特定代码块的执行时长。
