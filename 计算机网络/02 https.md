...
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| 2. 客户端用中间人的公钥加密生成`加密后的神秘字符`，中间人再次截获，并用其私钥破解了客户端的`加密后的神秘字符`。<br>破解后不直接发给服务端，而是修改`神秘字符`的内容，并且用服务端给的公钥加密修改后的`神秘字符`，再发送给服务端 |                                                              |
| 3. 服务端收到`加密后的神秘字符`后用私钥解密得到`神秘字符`    |                                                              |

通信在神不知鬼不觉的情况下被篡改了

![image-20240826121715567](02 https.assets/image-20240826121715567.png)

客户端和服务端之间并没有直接生成任何“会话密钥”，中间人可以随意修改通信内容。因此前面非对称和对称加密 加了个寂寞。
...
- 问题：客户端如何相信`数字证书`是由信任的CA颁发的呢？
  - `数字证书`上有CA的签名就可



![image-20240826123716011](02 https.assets/image-20240826123716011.png)

1. CA收到服务端的信息以及公钥后，对这些信息以及公钥进行`哈希运算`，得到一串较短的哈希字符（注：进行哈希运算并非进行了加密运算）
2. 在哈希运算后，CA自己也生成一对专门用于`数字证书`的公钥和私钥
3. **用`私钥`给`哈希字符`加密，得到`数字签名`**
4. CA把含有CA`数字签名`的`数字证书`发送给服务端
4. 虽然`数字证书`里的服务端信息都是明文的，但是核心在于CA的`数字签名`（若证书里的内容被篡改或服务端的公钥被修改了，生成的哈希值会不一样，最终的`数字签名`会变成另外的一串字符）





![image-20240826124525242](02 https.assets/image-20240826124525242.png)

1. 当客户端访问服务端，服务端会把含有CA`数字签名`的`数字证书`发给客户端
2. 为证明没有被中间人更改过服务端的公钥，客户端需要用CA签名时候生成的公钥（？？？？？？？？CA公钥传给客户端了？？什么时候传的），给`数字证书`里面的`数字签名`解密，得到一串哈希字符（`数字签名`加密前就是一串哈希字符）
3. 客户端对收到的`数字证书`内容进行同样的哈希运算，得到一串哈希字符
4. 若前后两串哈希字符一致，说明`数字证书`是靠谱的，没有被篡改



上述流程遗留的问题：客户端如何安全地拿到信任CA的公钥呢？

![image-20240826150349906](02 https.assets/image-20240826150349906.png)

1. 当服务端向第三方CA申请证书的时候，CA会生成专门给这个网站颁发证书的一对公钥和私钥，并用私钥给证书签名
2. 其实CA也需要`数字证书`来证明自己的身份，因此CA会把自己的公钥放在自己的`数字证书`里面。
3. 按照`数字证书`的生成原理，这份`数字证书`同样也需要另外一把私钥来进行签名。这就需要再加一层了，也就是`根CA`。
4. 网站申请证书的这个CA则是`中间CA`
5. 根CA同样需要生成一对专用的公钥和私钥，并且用这把私钥为中间CA的证书签名
6. 根CA既然有这把公钥，也就是说：根CA也有自己的证书。但是根CA的证书由谁来签名呢？
7. 根CA需要为自己的证书签名，然后**在用户的操作系统和浏览器里面预先安装根CA的证书**
8. 这样就可以闭合这一条证书链了
