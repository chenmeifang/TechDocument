...
export function render(
  ...
  // 检测container是否是符合要求的渲染容器
  // 即检测container是否是真实的DOM对象
  invariant(
...
![image-20240905191221563](06源码学习.assets/image-20240905191221563.png)
...
> **该方法做的最核心的事情：初始化Fiber数据结构——创建`fiberRoot`以及`rootFiber`**
...
function legacyRenderSubtreeIntoContainer(
    ...
    if (!root) {
        ...
        if (typeof callback === 'function') {
            // 改变callback函数中的this指向
            // 使其指向render方法第一个参数所对应的真实DOM对象
...
export function createFiberRoot(
  ...
  // 2.创建根节点对应的rootFiber——id为root的div所对应的Fiber对象
  ...
  // 5.为fiber对象添加updateQueue属性，初始化updateQueue对象
...
> 在updateContainer方法中，做的最核心的事情：**创建一个任务对象。**
>
...
![image-20240905220856670](06源码学习.assets/image-20240905220856670.png)
...
export function updateContainer(
  ...
  // 计算过期时间。为防止任务因为优先级的原因一直被打断而未能执行，react会设置一个过期时间，当时间到了过期时间的时候，如果任务还未执行的话，react将会强制执行该任务。
  ...
  // 4.将update对象加入到当前Fiber的更新队列当中（updateQueue）。待执行的任务都会被存储在fiber.updateQueue.shared.pending中
...
export function scheduleUpdateOnFiber(
  ...
  checkForNestedUpdates(); // 检查是否是无限循环的update。在componentWillUpdate或者componentDIdUpdate生命周期函数中重复调用setState方法时，可能会发生这种情况，React限制了嵌套更新的数量以防止无限循环。限制的嵌套更新数量为50，可通过NESTED_UPDATE_LIMIT全局变量获取
...
function performSyncWorkOnRoot(root) {
  ...
  if (workInProgress !== null) {
    ...
    do {
      try {
        workLoopSync(); // 以同步的方式开始构建Fiber对象
        break;
      } catch (thrownValue) {
...
#### [2. workLoopSync](https://www.bilibili.com/video/BV13j411D7K6?p=15&spm_id_from=pageDriver&vd_source=a7089a0e007e4167b4a61ef53acc6f7e)
...
#### [3. performUnitOfWork](https://www.bilibili.com/video/BV13j411D7K6?p=16)
...
#### [5. reconcileChildren](https://www.bilibili.com/video/BV13j411D7K6?p=17&spm_id_from=pageDriver&vd_source=a7089a0e007e4167b4a61ef53acc6f7e)
...
#### [6. completeUnitOfWork](https://www.bilibili.com/video/BV13j411D7K6?p=20&spm_id_from=pageDriver&vd_source=a7089a0e007e4167b4a61ef53acc6f7e)
