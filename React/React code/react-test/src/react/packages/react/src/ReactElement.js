...
export function createElement(type, config, children) {
  ...
  // Reserved names are extracted
  const props = {}; // 存储React Element中的普通元素属性 即不包含 key ref self source

  // React内部为了实现某些功能而存在的属性
  let key = null;
  let ref = null;
  let self = null;
  let source = null;
  ...
  // 由于从第三个参数开始及以后都表示子元素，所以减去前两个参数的结果就是子元素的数量
   ...
   * 遍历 defaultProps 对象，查看对象的 props 属性的值是否为 undefined
   * 如果为undefined，就将默认值赋值给对应的 props 属性值
   */
  // Resolve default props
  // 将type属性值视为函数，查看其中是否具有 defaultProps 属性
  if (type && type.defaultProps) {
    const defaultProps = type.defaultProps;
    for (propName in defaultProps) {
      if (props[propName] === undefined) {
